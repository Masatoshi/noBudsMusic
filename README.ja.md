...
Bluetoothイヤホンをタップしたとき、または再接続したときに、意図しない Music.app の起動を防ぐ常駐アプリです。
...
このプロジェクトは、作者自身が同じ問題に困ったことから始まりました。Bluetoothヘッドセットを
接続しただけで、再生するつもりがないのにMusic.appが起動しました。問題は音が流れることだけ
ではなく、起動後に何が起きるかに関係なく、接続だけでアプリが起動すること自体でした。
